...
Use it like: `conda env create -f env.yml`. If `opencv` is causing any problems, remove the `  - opencv-python=4.5.5.62` line in `env.yml` and try installing again. There is also another environment that is used specifically for interacting with the [gym_pcgrl](https://github.com/amidos2006/gym-pcgrl) codebase. If that is something you want to do, then create another environment from the `env_pcgrl.yml` file.
