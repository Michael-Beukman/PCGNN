...
class State:
    def __init__(self, simulate_enemies: bool = False):
        self.solid = []
        self.player = None
        self.exit = -1
    ...
    def check_enemies(self):
        """
            Checks if Mario is touching an enemy.
        """
    ...
    def move_enemies(self):
        ...
        """
        for e in self.enemies:
            new_pos = (e.x + e.direction, e.y)
            if self.checkMovableLocation(new_pos[0], new_pos[1]):
                e.x = new_pos[0]
                e.y = new_pos[1]
            else:
                e.steps = -1
    ...
    def update(self, dirX, dirY):
        ...
        if dirY < 0:
            ...
            dirY=0
        ground = False
        if self.player["y"] < len(self.solid) - 1 and self.player["y"] >= -1 and self.player["x"] < len(self.solid[self.player["y"] + 1]):
    ...
    def checkLose(self):
        return self.player["y"] >= self.height or self.has_lost
        ...
        for y in range(self.height):
            for x in range(self.width):
                if self.solid[y][x]:
                    ...
                    exit=self.exit==x
                    has_enemy = any([e.x == x and e.y == y for e in self.enemies])
                    if player:
                        if exit:
                            result += "w"
                        else:
